...
def test_scope_def_resolves : Bool :=
    ...
    let def_decl : Def := Def.mk defname (Term.type_ 1) Term.hole empty_constraints empty_attrs in
    let decls : List Decl := List.cons (Decl.def_d def_decl) empty_decls_list in
...
def test_scope_inductive_found : Bool :=
    ...
    let ind : Inductive := Inductive.mk color_path empty_params (Term.type_ 1) constructors empty_attrs in
    let decls : List Decl := List.cons (Decl.inductive_d ind) empty_decls_list in
    let sd : ScopeData := build_scope_from_decls path decls in
    let scope : Scope := make_scope sd in
