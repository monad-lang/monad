/// Benchmark: `List`+linear-scan vs `std/map.mo`'s `BTreeMap`, at sizes
/// approximating `lang/scope.mo`'s real `ScopeData.def_refs` usage.
///
...
/// collection sizes actually reached — `BTreeMap` here is a real,
/// allocation-heavy AVL tree (every `insert` builds fresh nodes
/// through rebalancing), not a cheap hash table. This benchmark
/// measures both the *build* cost (matching `scope_data_add_def`'s
/// repeated-cons / a repeated-`Map.insert` pattern) and the *lookup*
/// cost (matching `find_def_in_list` / `Map.lookup`) separately, at a
/// few sizes bracketing a real merged prelude+init `ScopeData`
/// (several hundred entries — init/ alone has ~388 top-level `def`s +
/// ~33 `type`/`struct` decls, each contributing more entries for their
/// own constructors).
///
...
def run_map_bench (n : I64) (label : String) : Bool :=
    ...
    let build_elapsed := I64.sub Bench.now build_start in
    let logged_build := Bench.report (String.concat "map  build  " label) build_elapsed in
    let lookup_start := Bench.now in
    let hits := map_lookup_range 0 n m 0 in
    let lookup_elapsed := I64.sub Bench.now lookup_start in
    let logged_lookup := Bench.report (String.concat "map  lookup " label) lookup_elapsed in
    I64.beq hits n
