...
def test_runtime_arity_exact : Bool :=
    let t := runtime_ir_text in
    // `monad_current_time` is deliberately NOT here: it used to be a
    // GENERATED stub returning 0, which made every `--verbose` timing in
    // a compiled binary read "0ms". It is now a real C function in
    ...
    check_contains t "define i64 @monad_string_starts_with(i64 %p0, i64 %p1) {"
        ...
        && Bool.not (check_contains t "define i64 @monad_current_time")
