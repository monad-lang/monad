...
def profile_to_list : IO Bool := do {
	...
	let _ <- Bench.report "to_list (100 bytes)" elapsed;
	return true
}
...
def profile_from_list : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "from_list (100 bytes)" elapsed;
	return true
}
...
def profile_concat_native : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "concat native (100+100 bytes)" elapsed;
	return true
}
...
def profile_length_native : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "length native (100 bytes)" elapsed;
	return true
}
...
def profile_contains_short : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "contains (100 bytes, short needle)" elapsed;
	return true
}
...
def profile_contains_miss : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "contains miss (100 bytes)" elapsed;
	return true
}
...
def profile_reverse : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "reverse (120 bytes)" elapsed;
	return true
}
...
def profile_trim : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "trim (50 bytes with padding)" elapsed;
	return true
}
...
def profile_repeat_10 : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "repeat 10x" elapsed;
	return true
}
...
def profile_starts_with_true : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "starts_with true (120 bytes)" elapsed;
	return true
}
...
def profile_starts_with_false : IO Bool := do {
	...
	let elapsed := I64.sub Bench.now start;
	let _ <- Bench.report "starts_with false (120 bytes)" elapsed;
	return true
}
